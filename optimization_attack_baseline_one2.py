import argparse
from attack_core.optimization_attack_baseline_one import universal_end2end_attack
...
min_acc = 100 
scale_loss =False
log_folder="./log__Unique_L2_final_2/" if p_norm==2 else "./log__Unique_SudotLinf_final_2/"
txt_config = dataset+"_conf"+str(int(confidence))+"_lamNorm_"+str(np.log2(lam_norm))+"_lam_"+str(lam)
text_tensorboard = "Unique_L2_"+txt_config if p_norm==2 else "Unique_SudotLinf_"+txt_config
...
if not os.path.isfile("saved_universals_optimization/"+text_tensorboard+".pt"):
    ...
    attack_instance(model,data, data_validation, dataloader.att, 50,confidence, num_epochs=500,\
                    loss_norm = p_norm, \
                    norm_coefficient=lam_norm, loss_coefficient=lam, normalized_perturbation=False, project_perturbation=True,\
