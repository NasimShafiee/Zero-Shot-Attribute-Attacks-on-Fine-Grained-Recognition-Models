import argparse
from attack_core.optimization_attack_dazle_test_ProjectLinf import universal_end2end_attack
...
parser.add_argument("--lamnorm",   default="0")
...
lambdas_2=[0]
lambdas_3=[0]
lambdas_4    = [0]#[2**float(args.lam4)]#[2**-7, 2**-5, 2**-3, 0.5, 2]
lambdas_norm = [0]#[2**float(args.lamnorm)]#[2**-7, 2**-5, 2**-3, 0.5, 2]
norm_perturbation = 2000/255 if p_norm==2 else 10/255
lambda_distance =0
data_indices= [3]
iteration=0
attention = False
...
scale_loss =False
log_folder="./log__CUAP_L2/" if p_norm==2 else "./log__CUAP_ProjLinf/"
txt_config = dataset+"_conf"+str(int(confidences[0]))+"_lam_"+str(lambdas[0])+"_"+str(lambdas_4[0])
text_tensorboard = "_L2_"+txt_config if p_norm==2 else "_ProjLinf_CE_"+txt_config
                                ...
                                for lam in lambdas:
                                    ...
                                    if not os.path.isfile("saved_universals_optimization/"+text_tensorboard+".pt"):
                                        ...
                                        attack_instance(model,data, data_validation, dataloader.att, dataloader.w2v_att, 50, confidence, num_epochs=500,\
                                                        loss_norm = p_norm, norm_perturbation=norm_perturbation,\
