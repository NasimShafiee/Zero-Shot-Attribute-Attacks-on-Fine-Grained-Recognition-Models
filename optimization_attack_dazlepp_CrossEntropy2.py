import argparse
from attack_core.optimization_attack_dazle_CrossEntropy import universal_end2end_attack
...
scale_loss =False
log_folder="./log__CUAPCrossEntropy_L2_final/" if p_norm==2 else "./log__CUAPCrossEntropy_SudotLinf_final/"
...
text_tensorboard = "_CUAP_CE_L2_"+txt_config if p_norm==2 else "_CUAP_CE_SudotLinf_"+txt_config
                            ...
                            for lam_4 in lambdas_4:
                                for lam in lambdas:
                                    ...
                                    if not os.path.isfile("saved_universals_optimization/"+text_tensorboard+".pt"):
                                        ...
                                        attack_instance(model,data, data_validation, dataloader.att, dataloader.w2v_att, 50, confidence, num_epochs=50,\
                                                        loss_norm = p_norm, \
                                    ...
                                    if p_norm ==2:
                                        norm__range = torch.arange(0,12,2)
                                        p_norm = torch.tensor(p_norm)
                                    else:
                                        norm__range  = torch.arange(0.0,0.12,0.02)
