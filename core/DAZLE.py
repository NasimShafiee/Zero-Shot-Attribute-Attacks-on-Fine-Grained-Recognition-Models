...
#        print('Class agnostic attribute attention with hidden att {}'.format(self.hidden))
    ...
    def compute_loss_rank(self,in_package):  
        ...
        s_c = torch.gather(S_pp,1,batch_label.view(-1,1))  
        if self.is_conservative:  
            S_seen = S_pp  
        else:  
...
#            V_n = torch.einsum('iv,i->iv',V_n,self.att_strength)
    ...
    def compute_attribute_embed(self,Hs):
        ...
        self.package_2 = {'S_pp':S_pp,'A_p':A_p, 'S_p':S_p}  
        
        return S_pp  
    
    def forward(self,Fs):
        Hs = self.extract_attention(Fs)
        package_1 = self.package_1

        S_pp = self.compute_attribute_embed(Hs)
        package_2 = self.package_2

        self.package_out = {'A':package_1['A'],'A_p':package_2['A_p'],'S_pp':package_2['S_pp'], 'S_p':package_2['S_p']} 
        
        log_softmax_func = nn.LogSoftmax(dim=1)
        S_pp           = log_softmax_func(S_pp)
        return S_pp