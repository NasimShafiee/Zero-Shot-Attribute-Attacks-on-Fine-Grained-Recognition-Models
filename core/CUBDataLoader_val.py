...
img_dir = os.path.join(NFS_path_AoA,'data/CUB/')
mat_path = os.path.join(NFS_path_AoA,'data/xlsa17/data/CUB/res101.mat')
...
class CUBDataLoader():
    ...
    def next_batch_img(self,batch_size):
        if self.is_balance:
            idx = []
        ...
        batch_feature = features[idx].to(self.device)
        batch_label =  labels[idx].to(self.device)
        batch_att = self.att[batch_label].to(self.device)
        batch_files = img_files[idx.cpu().numpy()]
        return batch_label, batch_feature,batch_files, batch_att
        
    def next_batch_img_obs(self, batch_size,class_id,is_trainset = False):
        features = None
        labels = None
        img_files = None
        if class_id in self.seenclasses.cpu():
            if is_trainset:
                features = self.data['train_seen']['resnet_features']
                labels = self.data['train_seen']['labels']
                img_files = self.data['train_seen']['img_path']
        ...
        elif class_id in self.unseenclasses.cpu():
...
#        pdb.set_trace()
        ...
        trainval_loc = np.array(hf.get('trainval_loc'))
        train_loc = np.array(hf.get('train_loc')) #--> train_feature = TRAIN SEEN
        val_unseen_loc = np.array(hf.get('val_unseen_loc')) 
        test_seen_loc = np.array(hf.get('test_seen_loc'))
        test_unseen_loc = np.array(hf.get('test_unseen_loc'))
