...
scale_loss =False
log_folder="./log__CUAP_L2/" if p_norm==2 else "./log__CUAP_Linf/"
                ...
                for lam_2 in lambdas_2:
                    for lam_3 in lambdas_3:
                        for lam_4 in lambdas_4:
                            for lam in lambdas:
                                txt_config = dataset+"_conf"+str(confidence)+"_lamNorm_"+str(np.log2(lam_norm))+"_lam_"+str(lam)+"_"+str(np.log2(lam_4))
                                text_tensorboard = "_L2_"+txt_config if p_norm==2 else "_Linf_"+txt_config
