...
class universal_end2end_attack(torch.nn.Module):
    def __init__(self, num_attributes, num_classes, image_size, w2v_size, dataset, w_as="function",\
        ...
        self.e        = torch.nn.Parameter(torch.zeros(1,\
    ...
    def forward(self, model, dataloader, dataloader_val, semantic_vector, batch_size, \
        ...
        for epoch in range(num_epochs):
            ...
            for i in range(0, dataset_length, batch_size):
                ...
                if loss_norm==2:
                    loss_1  = 0.5 * torch.norm(self.e.reshape(self.e.size(0),-1), p=loss_norm, dim=-1).pow(loss_norm).sum()#/ (self.num_attributes)
                else:
                    # loss_1  = 0.5 * torch.norm(self.e.reshape(self.e.size(0),-1), p=loss_norm, dim=-1).sum()
                    t = (0.005) / 0.224
                    loss_1  = torch.einsum('achw,achw->achw', (self.e.abs() - t )>0 , self.e.abs() - t ).sum()
                ...
                loss.backward()
                clipping_value = 1 
                torch.nn.utils.clip_grad_norm_([self.e], clipping_value)
            ...
            with torch.no_grad():
                ...
                for i in range(0, dataset_length_val, batch_size):
                    ...
                    # Calculate Loss function
                    if loss_norm==2:
                        loss_1  = 0.5 * torch.norm(self.e.reshape(self.e.size(0),-1), p=loss_norm, dim=-1).pow(loss_norm).sum()
                    else:
                        # loss_1  = 0.5 * torch.norm(self.e.reshape(self.e.size(0),-1), p=loss_norm, dim=-1).sum()
                        t = (0.005 )/0.224
                        loss_1  = torch.einsum('achw,achw->achw', (self.e.abs() - t )>0 , self.e.abs() - t ).sum()
