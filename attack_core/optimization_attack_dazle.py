...
class universal_end2end_attack(torch.nn.Module):
    ...
    def add_perturbation(self, normalized_batch_image, perturbation, normalized_perturbation=True, project_perturbation=False, scale=True, norm_scale=1, norm_perturbation=1):
        if normalized_perturbation:
            
            if scale:# Scale Perturbation
                perturbation = transform(self.scale_perturbation(inverse_transform(perturbation), norm_scale, norm_perturbation))
            ...
            if scale:# Scale Perturbation
                perturbation = self.scale_perturbation(perturbation, norm_scale, norm_perturbation)
    ...
    def scale_perturbation(self, perturbation, norm_scale, perturbation_scale):
        # Magnitude of Perturbations
        norm_scale_img   = torch.norm(perturbation,p=norm_scale,dim=(1,2,3)).mean()

        # Scaling Perturbations
        scale_l = torch.tensor(perturbation_scale, requires_grad=False)
        ratio_l = norm_scale_img/scale_l
        if ratio_l==0:
            ratio_l=torch.zeros_like(ratio_l)+0.00001
    ...
    def image_visualization(self, images, perturbations, adv_images, adv_title, cln_title, attack_scenario, norm_perturbation, norm_scale, validation=False):
        ...
        fig.tight_layout()
        self.logger.add_figure('True attack/'+attack_scenario+str(norm_perturbation)+'_'+str(norm_scale.item()), fig, int(not(validation)))
    ...
    def forward(self, model, dataloader, dataloader_val, semantic_vector, w2v_vector, batch_size, confidence, num_epochs=10, loss_norm=2, norm_coefficient=1, loss_coefficient=10, reg_e_coefficient=0.001,reg_w_coefficient=0.001, reg_utility=0.001, reg_dist=0, normalized_perturbation=True, project_perturbation=False, checkpoint_path="./"):
        ...
        for epoch in range(num_epochs):
            ...
            for i in range(0, dataset_length, batch_size):            
                ...
                if not self.partition:
                    ...
                    # perturbation    = self.scale_perturbation(perturbation, norm_scale=2, perturbation_scale=2)
                else:
                ...
                if not self.scale_loss:
                    ...
                    adv_batch_image = self.add_perturbation(corr_batch_image.reshape(-1,3,224,224),\
                                                            perturbation,\
                                                            normalized_perturbation=normalized_perturbation,\
                                                            project_perturbation=project_perturbation, scale=True, norm_scale=1, norm_perturbation=100)
    ...
    def attack_with_universals(self, model, dataloader, semantic_vector, w2v_vector,\
                               batch_size, norm_perturbation=1, norm_scale=1, attack_scenario="seen", normalized_perturbation=True, project_perturbation=False, validation=False):
        with torch.no_grad():
            ...
            for i in range(0, length_dataset, batch_size):
                ...
                if i==0:
                    ...
                    self.image_visualization(inverse_transform(batch_image), inverse_transform(batch_perturbation),\
                                             inverse_transform(batch_adv_image), adv_title, cln_title, attack_scenario, norm_perturbation, norm_scale,validation)
                    ...
                    self.visualize_attention(images=batch_adv_image, labels=adv_pred, attentions_map=model.dazle.package_out['A'], attentions_attribute=model.dazle.package_out['A_p'],\
                                             n_top_attr=5, mode="adv", norm=norm_scale)
            ...
            self.logger.add_scalar('true avg per pixel/'+attack_scenario+str(norm_perturbation)+'_'+str(norm_scale.item()), avg_per_pixel_perturbation,norm_scale)
            self.logger.add_scalar('true model accuracy/'+attack_scenario+str(norm_perturbation)+'_'+str(norm_scale.item()), model_accuracy/num_attacked_samples, norm_scale)
            self.logger.add_scalar('true model initial accuracy/'+attack_scenario+str(norm_perturbation)+'_'+str(norm_scale.item()), model_initial_accuracy/num_attacked_samples, norm_scale)
